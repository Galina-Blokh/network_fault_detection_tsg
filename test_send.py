...
NUM_OF_PACK_TO_SNIFF = 5
PSW = "Vitalina1882"
USRNANE = "galin"
HOST_IP = "192.168.43.159"  # for ssh connection
SRC_IP = "192.168.43.121"
SRC_PATH = './my_csv_10.csv' #./my_csv_3_2_4000.csv
DST_PATH = './Downloads/my_csv_10.csv'
...
def build_csv_file_with_name(df, src_path=SRC_PATH, dst_path=DST_PATH):
    """
    Transforms the df into csv file and sends it with SFTP client to remote host (to local machine)
    ...
    s.connect(HOST_IP, 22, username=USRNANE, password=PSW, timeout=15, allow_agent=True)
    sftp = s.open_sftp()
    print('open sftp...')
...
def main(num=NUM_OF_PACK_TO_SNIFF):
    ...
    while n < num:
        for i in (['159', '41']):
            delay.append(collect_the_data_for_one_host('192.168.43.{}'.format(i))[0])
            srcs.append(collect_the_data_for_one_host('192.168.43.{}'.format(i))[1])
            dsts.append(collect_the_data_for_one_host('192.168.43.{}'.format(i))[2])
            ttls.append(collect_the_data_for_one_host('192.168.43.{}'.format(i))[3])
            dst_time.append(collect_the_data_for_one_host('192.168.43.{}'.format(i))[4])
            n += 1
